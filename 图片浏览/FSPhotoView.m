...
- (void)normalModeShowWithSenderView:(UIView*)senderView
{
    ...
    if (oriFrame.origin.y<44)
    {
        ...
        UIView *clipsView = [[UIView alloc] initWithFrame:clipRect];
...
#warning 此处最重要，此处是为了动画显示所有的图片
    if (senderView.animationImages) {
        self.imageView.contentMode = UIViewContentModeScaleAspectFill;
        self.imageView.animationImages = senderView.animationImages;
...
- (void)aspectFillModeShowWithSenderView:(UIView*)senderView
{
    ...
    UIView *clipsView = [[UIView alloc] initWithFrame:clipFrame];
    [clipsView setClipsToBounds:YES];
    [[self imageView] removeFromSuperview];
    [clipsView addSubview:self.imageView];
    [self addSubview:clipsView];
...
- (void)removePhotoView
{
    ...
    {
        UIView *clipView = [[UIView alloc] initWithFrame:[self imageView].frame];
